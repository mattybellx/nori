...
[![Dependencies: stdlib only](https://img.shields.io/badge/dependencies-stdlib%20only-4caf50)](deepseek_engine/pyproject.toml)
[![Tests: 124 passing](https://img.shields.io/badge/tests-124%20passing-success)](deepseek_engine/tests)
...
- **Evidence-driven, not claimed** — every strategy decision is traced to
  peer-reviewed research in [RESEARCH.md](deepseek_engine/RESEARCH.md); the
  design contract is [ARCHITECTURE.md](deepseek_engine/ARCHITECTURE.md);
  measured results — including honest negative results — are in
...
[`deepseek_engine/.env.example`](deepseek_engine/.env.example) — copy it to
`.env`).
...
A few headline findings from
[BENCHMARKS.md](deepseek_engine/BENCHMARKS.md) — every number ships with
seed, n, model and hardware:
...
Full details, including negative results and experimental noise studies, are
in [BENCHMARKS.md](deepseek_engine/BENCHMARKS.md); known limitations are in
[LIMITATIONS.md](deepseek_engine/LIMITATIONS.md).
...
- [ARCHITECTURE.md](deepseek_engine/ARCHITECTURE.md) — design contract and module map
- [BENCHMARKS.md](deepseek_engine/BENCHMARKS.md) — measured results, methodology, negative results
- [LIMITATIONS.md](deepseek_engine/LIMITATIONS.md) — honest limits and known quirks
- [RESEARCH.md](deepseek_engine/RESEARCH.md) — evidence base, papers, and decision trace
...
`deepseek_engine/sessions/*.jsonl` files, your key in `settings.json` (or your
`.env`). Nothing is sent anywhere except to the model provider you configured.
